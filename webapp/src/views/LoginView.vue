...
const hasOAuthProviders = computed(() =>
  config.googleAuthentication || config.ovhAuthentication || config.oidcAuthentication || config.githubAuthentication
)
...
async function githubLogin() {
  try {
    const resp = await fetch(window.location.origin + window.location.pathname.replace(/\/$/, '') + '/auth/github/login')
    if (!resp.ok) throw new Error('GitHub login failed')
    const url = await resp.text()
    window.location.href = url
  } catch (err) {
    ...
    <div class="w-full max-w-sm">
      ...
      <div class="glass-card p-6 space-y-5">
        ...
        <div v-if="hasOAuthProviders"
          ...
          <button v-if="config.githubAuthentication"
                  @click="githubLogin"
                  class="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border border-surface-600
                         bg-surface-700/50 text-surface-200 hover:bg-surface-600/50 hover:text-surface-100
                         transition-all text-sm font-medium">
